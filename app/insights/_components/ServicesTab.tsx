  ...
  return (
    <>
      <p className="text-xs" style={{ color: "var(--text-muted)" }}>
        Service catalog usage embedded on appointment employees.{" "}
        <InlineLink href="/services">Open services dashboard →</InlineLink>
      </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatTile label="Booked service-line value" value={formatCurrency(bookedValue)} />
        <StatTile label="Completed service-line value" value={formatCurrency(completedValue)} tone="good" />
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatTile label="Employees with services" value={formatNumber(data.employeesWithServices)} tone="good" />
        <StatTile
          label="Employees without services"
          value={formatNumber(data.employeesWithoutServices)}
          tone={data.employeesWithoutServices ? "warning" : "good"}
        />
      </div>
      ...
      <SectionCard title="Recent appointments with services">
        {data.recent.length === 0 ? (
          <EmptyState title="No recent service appointments" />
        ) : (
          <div className="flex flex-col gap-3">
            {data.recent.slice(0, 8).map((appointment) => (
              <div key={appointment._id} className="rounded p-3" style={{ border: "1px solid var(--gridline)" }}>
                ...
                <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                  {appointment.details?.company?.name ?? "-"} · {appointment.details?.clinic ?? "-"} ·{" "}
                  {displayDate(appointment.tracking?.[0]?.date)}
                </p>
              </div>
            ))}
          </div>
